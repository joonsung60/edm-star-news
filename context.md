...
lib/edm-entities.json의 artists_top500_relevance_2024_2025 각 항목에
weight 필드가 명시되어 있다. rank 기반으로 사전 계산된 값이다.

- rank 1~100: weight 1.5 (최상위 아티스트)
- rank 101~300: weight 1.0 (중간)
- rank 301+: weight 0.7 (하위)
- rank 없음: weight 0.8 (기본값)

이 weight는 suggest 클러스터 스코어링 시 엔티티 중요도로 사용된다.
loadEntityDictionary에서 JSON의 weight 필드를 직접 읽는다.
